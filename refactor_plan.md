...
**الدوال الرئيسية / Main Functions:**
- `make_category()` - إنشاء تصنيف
- `new_category()` - تصنيف جديد
- `Make_temp()` - إنشاء قالب
- `Make_Portal()` - إنشاء بوابة
- `make_text()` - إنشاء نص التصنيف
- `ar_make_lab()` - إنشاء تسمية عربية
- `make_ar()` - إنشاء تصنيف عربي
- `check_en_temps()` - فحص القوالب الإنجليزية
- `getP373()` - الحصول على P373 من ويكي بيانات

**خطة الاختبار / Test Plan:**
- [ ] اختبار إنشاء صفحات تصنيف جديدة
- [ ] اختبار إنشاء نص التصنيف مع Wikidata
- [ ] اختبار إنشاء القوالب والبوابات
- [ ] اختبار فحص القوالب الإنجليزية
- [ ] اختبار الحصول على P373 من Wikidata
- [ ] اختبار التحقق من وجود العناوين العربية
- [ ] Mock استدعاءات Wikidata API
- [ ] اختبار معالجة قوائم التصنيفات
...
- [ ] wiki_api (استدعاءات API الأساسية)
- [ ] b18_new (معالجة التصنيفات)
- [ ] c18_new (أدوات التصنيفات)
...
الأولويات:
1. **عالية جداً:** wiki_api, b18_new, c18_new
2. **عالية:** api_sql, wd_bots, mk_cats  
3. **متوسطة:** helps, temp
4. **منخفضة:** utils
...
**الحالة / Status:** 🟡 في انتظار التنفيذ / Pending Implementation
