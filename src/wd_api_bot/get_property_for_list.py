...
def get_property_label_for_qids(properties, List):
    # ---
    # logger.output('start get_property_label_for_qids:' )
    # ---
    ...
    for qid in List:
        ...
        for prop in properties:
            lino += f"|{prop}"
        lino += "}}\n"
        # if num == 1 : logger.output(lino)
        text += lino
    # ---
    if "printprase" in sys.argv:
        print(text)
    # ---
    jso = himoBOT2.Parse_Text(text, title, "ar")
    # ---
    if not jso:
        logger.output('get_property_label_for_qids: jso == ""')
        return False
    # ---
    elif jso == text:
        logger.output("<<lightred>> get_property_label_for_qids: jso == text ")
        return False
    # ---
    newtabe = himoBOT2.load_SPARQL_New(sparql="{\n" + jso + '\n"cdcdcd":{}\n}')
    # ---
    if newtabe:
        del newtabe["cdcdcd"]
    # ---
    # logger.output( newtabe )
    # ---
    return newtabe
