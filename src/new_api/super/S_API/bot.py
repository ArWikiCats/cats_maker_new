...
class BOTS_APIS(HANDEL_ERRORS, ASK_BOT):
    ...
    def move(self, old_title, to, reason="", noredirect=False, movesubpages=False, return_dict=False):
        ...
        if error:
            ...
            if error_code == "articleexists":
                logger.debug("<<red>> articleexists")
    ...
    def upload_by_file(self, file_name, text, file_path, comment="", ignorewarnings=False):
        ...
        # ---
        logger.debug(f"<<lightyellow>> {file_path=}...")
        # ---
        params = {
            "action": "upload",
            "format": "json",
        ...
        # ---
        if success:
            logger.warning(f"<<lightgreen>> ** upload true .. [[File:{file_name}]] ")
            return True
        # ---
        if duplicate:
            logger.debug(f"<<lightred>> ** duplicate file: {duplicate}.")
        # ---
        return data
